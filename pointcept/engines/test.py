        ...
        for idx, input_dict in enumerate(self.test_loader):
            ...
            for i in range(len(fragment_list)):
                ...
                with torch.no_grad():
                    ...
                    pred_coord = output_dict["pred_coord"]

                    # Chamfer Distance 計算（chunkで処理）
                    chamfer = self.compute_chamfer(pred_coord, gt_coord)
                    chamfer_meter.update(chamfer)

                    logger.info(f"Test [{idx+1}/{len(self.test_loader)}] {name} Chamfer: {chamfer:.4f}")

                    np.save(os.path.join(save_path, f"{name}_pred.npy"), pred_coord.cpu().numpy())

        logger.info("<<<<<<<<<<<<<<<<< End Evaluation (PCD Completion) <<<<<<<<<<<<<<<<<")
        logger.info(f"Final AvgChamfer: {chamfer_meter.avg:.4f}")

    @staticmethod
    def compute_chamfer(pred, target, chunk_size=4096):
        min1 = []
        ...
        return (min1.mean() + min2.mean()).item()
